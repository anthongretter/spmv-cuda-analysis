...
# Configuration - Larger and more diverse matrices
MATRIX_CONFIG = [
    {
        "name": "small_grid_2d",
        "description": "Small 2D grid (5-point stencil)",
        "expected_size": "small",
        "generate": True,
        "params": {"type": "grid", "size": 50}
    },
    {
        "name": "medium_grid_2d",
        "description": "Medium 2D grid (5-point stencil)",
        "expected_size": "medium",
        "generate": True,
        "params": {"type": "grid", "size": 200}
    },
    {
        "name": "large_grid_2d",
        "description": "Large 2D grid (5-point stencil)",
        "expected_size": "large",
        "generate": True,
        "params": {"type": "grid", "size": 400}
    },
    {
        "name": "small_random",
        "description": "Small random sparse matrix",
        "expected_size": "small",
        "generate": True,
        "params": {"type": "random", "rows": 2000, "cols": 2000, "density": 0.005}
    },
    {
        "name": "medium_random",
        "description": "Medium random sparse matrix",
        "expected_size": "medium", 
        "generate": True,
        "params": {"type": "random", "rows": 10000, "cols": 10000, "density": 0.001}
    },
    {
        "name": "large_random",
        "description": "Large random sparse matrix",
        "expected_size": "large", 
        "generate": True,
        "params": {"type": "random", "rows": 25000, "cols": 25000, "density": 0.0005}
    },
    {
        "name": "narrow_band",
        "description": "Narrow banded matrix (tridiagonal-like)",
        "expected_size": "small",
        "generate": True,
        "params": {"type": "band", "size": 5000, "bandwidth": 3}
    },
    {
        "name": "wide_band",
        "description": "Wide banded matrix (pentadiagonal-like)",
        "expected_size": "medium",
        "generate": True,
        "params": {"type": "band", "size": 8000, "bandwidth": 100}
    },
    {
        "name": "very_wide_band",
        "description": "Very wide banded matrix",
        "expected_size": "large",
        "generate": True,
        "params": {"type": "band", "size": 15000, "bandwidth": 500}
    },
    {
        "name": "small_power_law",
        "description": "Small power-law distribution (scale-free)",
        "expected_size": "small",
        "generate": True,
        "params": {"type": "power_law", "rows": 3000, "cols": 3000, "alpha": 2.0}
    },
    {
        "name": "medium_power_law",
        "description": "Medium power-law distribution (social network-like)",
        "expected_size": "medium",
        "generate": True,
        "params": {"type": "power_law", "rows": 12000, "cols": 12000, "alpha": 2.5}
    },
    {
        "name": "large_power_law",
        "description": "Large power-law distribution (very irregular)",
        "expected_size": "large",
        "generate": True,
        "params": {"type": "power_law", "rows": 20000, "cols": 20000, "alpha": 3.0}
    }
]
...
class MatrixGenerator:
    ...
    def generate_matrix(self, config):
        """Generate synthetic matrix based on configuration"""
        filename = f"{config['name']}.mtx"
        filepath = self.base_dir / filename
        
        if filepath.exists():
            print(f"✓ Matrix {config['name']} already exists")
            return str(filepath)
        
        if not self.has_modules:
            print(f"❌ Cannot generate {config['name']}: missing Python modules")
            return None
        ...
        try:
            ...
            if params['type'] == 'grid':
                return self._generate_grid_matrix(filepath, params)
            elif params['type'] == 'random':
                return self._generate_random_matrix(filepath, params)
            elif params['type'] == 'band':
                return self._generate_band_matrix(filepath, params)
            elif params['type'] == 'power_law':
                return self._generate_power_law_matrix(filepath, params)
            else:
                print(f"❌ Unknown matrix type: {params['type']}")
                return None
    ...
    def _generate_grid_matrix(self, filepath, params):
        """Generate 5-point stencil grid matrix"""
        import random
        size = params['size']
        n = size * size
        ...
        for i in range(size):
            for j in range(size):
                ...
                # Up neighbor
                if i > 0:
                    entries.append((row, row - size, -1.0))
                
                # Down neighbor
                if i < size - 1:
                    entries.append((row, row + size, -1.0))
        
        return self._write_mtx_file(filepath, n, n, entries)
    
    def _generate_random_matrix(self, filepath, params):
        ...
        return self._write_mtx_file(filepath, rows, cols, entries)
    
    def _generate_band_matrix(self, filepath, params):
        ...
        for i in range(size):
            for j in range(max(0, i - bandwidth), min(size, i + bandwidth + 1)):
                if i != j or random.random() > 0.1:  # Some sparsity
                    value = random.uniform(1.0, 10.0)
                    entries.append((i, j, value))
        
        return self._write_mtx_file(filepath, size, size, entries)
    
    def _generate_power_law_matrix(self, filepath, params):
        ...
        # Generate power-law distributed row lengths
        for i in range(rows):
            # Power-law distribution for number of non-zeros in row
            u = random.random()
            row_nnz = int(1.0 / (u ** (1.0 / alpha))) 
            row_nnz = min(row_nnz, cols // 4)  # Cap to reasonable size
            row_nnz = max(row_nnz, 1)  # At least one entry
            
            # Randomly place non-zeros in this row
            col_indices = random.sample(range(cols), row_nnz)
    ...
    def _write_mtx_file(self, filepath, rows, cols, entries):
        """Write entries to Matrix Market format file"""
        try:
            with open(filepath, 'w') as f:
                ...
                for row, col, val in entries:
                    f.write(f"{row + 1} {col + 1} {val:.6f}\n")
            
            print(f"✅ Successfully generated {filepath.name}")
            return str(filepath)
            
        except Exception as e:
            print(f"❌ Failed to write matrix file: {e}")
            return None
    ...
    def generate_all(self):
        ...
        for config in MATRIX_CONFIG:
            if config.get('generate', False):
                filepath = self.generate_matrix(config)
                if filepath:
                    generated.append((config, filepath))
            else:
                print(f"⚠️ Skipping non-generatable matrix: {config['name']}")
        return generated
...
def main():
    ...
    matrices = generator.generate_all()
    
    if not matrices:
        print("❌ No matrices generated. Exiting.")
        return
    
    print(f"✅ Successfully generated {len(matrices)} matrices")
    
    # Step 2: Run benchmarks
    print("\n🔬 Step 2: Running benchmarks...")
    benchmark = SpMVBenchmark()
    benchmark.benchmark_all(matrices)
